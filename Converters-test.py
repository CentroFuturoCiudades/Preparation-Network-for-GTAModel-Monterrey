...
class Converters():
        ...
        with open(path, encoding='utf-8', errors='ignore') as net:
            for line in net:
                ...
                if line.startswith(self.TABLE_NAME_HEADER):
                    # new table
                    _table_name = line.split(self.ATTR_DELIMITER)[-1].strip().replace(' ', '_')
    ...
    def csv2net(self,
        ...
        :return: None
        """
        if csv_path == None or not isinstance(csv_path, str):
            print('[Error] You need to specify a validate input path ')
            return
        else:
            if csv_path[-1] != '/':
                csv_path += '/'
        if net_path == None:
            net_path = csv_path + 'output.net'
        print(net_path)
        
        with open(net_path, mode='w', encoding='utf-8', errors='ignore') as net:
            #write header
            header = ['\ufeff$VISION\n', '* U of Toronto\n', '* 08/04/23\n']
            net.writelines(header)
            csvF = glob(csv_path + '*.csv')
            #write tables
            for f in csvF:
                tab_head = '* Table: ' + os.path.basename(f).split('.')[0].replace('_', ' ') + '\n'
                #table header
                net.writelines(['* ' +self.NEWLINE_MAC,
                                tab_head,
                                '* ' +self.NEWLINE_MAC])
                #columns header
                words = os.path.basename(f).split('.')[0].split('_')
                words[-1] = Word(words[-1]).singularize()
                col_def = ''.join(words).upper().replace('-','')    
                df = pd.read_csv(f)
                #remove nan and format decimal places
                df = df.fillna('')
                net.writelines('$' + col_def + ':' + ';'.join(map(str, df.columns.values.tolist())))
                #row data
                net.writelines([';'.join(map(self.__setPrec, l)) + self.NEWLINE_MAC for l in df.values.tolist()])
                #newline to split tables
                net.write(self.NEWLINE_MAC)
        net.close()
        return None
